...
const TERMS_ACCEPTED_KEY = '@alchemize_terms_accepted';
const TERMS_VERSION = '1.0.0';

// Apple reviewer test account — seeded on every launch
...
async function seedAppleReviewerAccount() {
  try {
    ...
    const already = users.some(u => u.email.toLowerCase() === APPLE_REVIEWER_EMAIL.toLowerCase());
    if (!already) {
      users.push({ id: 'apple_reviewer_account', email: APPLE_REVIEWER_EMAIL, name: APPLE_REVIEWER_NAME, password: APPLE_REVIEWER_PASSWORD });
...
export const [AuthProvider, useAuth] = createContextHook(() => {
  ...
  useEffect(() => {
    async function loadAuthState() {
      try {
        ...
        if (storedAuth) {
          try {
            ...
            if (parsed?.user && parsed?.token) {
              setAuthState(parsed);
              if (parsed.user.id) await setCurrentUserId(parsed.user.id);
        ...
        if (storedRememberMe === 'true') setRememberMeState(true);
      } catch (error) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    try {
      ...
      if (!user) return { success: false, error: 'No account found. Please sign up first.' };
      ...
      const authData: AuthState = { user: { id: user.id, email: user.email, name: user.name }, token };
  ...
  const signup = useCallback(async (email: string, password: string, name: string) => {
    try {
      ...
      await AsyncStorage.setItem(USERS_STORAGE_KEY, JSON.stringify(users));
      const token = `token_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const authData: AuthState = { user: { id: userId, email, name }, token };
      await AsyncStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify(authData));
  ...
  const loginWithApple = useCallback(async () => {
    try {
      ...
      const name = credential.fullName ? `${credential.fullName.givenName ?? ''} ${credential.fullName.familyName ?? ''}`.trim() : 'Apple User';
      const usersData = await AsyncStorage.getItem(USERS_STORAGE_KEY);
      const users: StoredUser[] = usersData ? JSON.parse(usersData) : [];
  ...
  const deleteAccount = useCallback(async () => {
    try {
      ...
      if (userId) {
        const usersData = await AsyncStorage.getItem(USERS_STORAGE_KEY);
        const users: StoredUser[] = usersData ? JSON.parse(usersData) : [];
        await AsyncStorage.setItem(USERS_STORAGE_KEY, JSON.stringify(users.filter(u => u.id !== userId)));
      }
      setAuthState({ user: null, token: null });
